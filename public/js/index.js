...
} from "./charts/final-graphs/firstFinalGraph.js";
import {
    redrawFirstByTimeGraph,
    drawFirstByTimeGraph,
} from "./charts/graphs-by-time/firstGraphByTime.js";
import {
    drawSecondByTimeGraph,
    redrawSecondByTimeGraph,
} from "./charts/graphs-by-time/secondGraphByTime.js";
...
let $cardDescriptionSymbols =
    document.getElementsByClassName("description-symbol");
...
let $cardGraphBubbleTriangles = document.getElementsByClassName(
    "graph-bubble-triangle"
);
...
let $selectionButtonTriangles = document.getElementsByClassName(
    "open-options__triangle"
);

const $pieChartGraphContainer = document.getElementById("third__final-graph");
const $finalBubbleButton = Array.from(
    document.getElementsByClassName("third__final-graph__open")
)[0];

const $accessibilityButton = document.getElementsByClassName(
...
const $firstTableSearchInput =
    document.getElementsByClassName("table-search-input")[0];

const $darkTealLegendCircle = Array.from(
    document.getElementsByClassName("dark-teal-circle")
);
const $greyLegendCircle = Array.from(
    document.getElementsByClassName("grey-circle")
);
const $lightBlueLegendCircle = Array.from(
    document.getElementsByClassName("light-blue-circle")
);
const $greenLegendCircle = Array.from(
    document.getElementsByClassName("green-circle")
);
...
for (let i = 0; i < $cardDescriptionSymbols.length; i++) {
    ...
    cardDescSymbol.addEventListener("mouseenter", () => {
        ...
        if (i % 2 === 1)
            cardDescBox.classList.add("left-sided__cards__description-box");
    ...
    cardDescSymbol.addEventListener("mouseleave", () => {
        ...
        if (
            i % 2 === 1 &&
            cardDescBox.classList.contains("left-sided__cards__description-box")
        )
            cardDescBox.classList.add("left-sided__cards__description-box");
...
for (let i = 0; i < $cardGraphButtons.length; i++) {
    ...
    cardGraphButton.addEventListener("click", (e) => {
        ...
        changeElementColors(
            cardGraphCircle,
            "card-graph-button__opened__accessability-mode"
        );
        changeElementColors(
            cardGraphCircle.querySelector("svg"),
            "card-graph-svg__opened__accessability-mode"
        );
...
function closingCardGraphBubble() {
    for (let i = 0; i < $cardGraphButtons.length; i++) {
        ...
        if (cardGraphButton.classList.contains("card-graph__opened"))
            cardGraphButton.classList.remove("card-graph__opened");
        if (card.classList.contains("card__graph__opened"))
            card.classList.remove("card__graph__opened");
        if (graphBubble.classList.contains("graph-bubble__opened"))
            graphBubble.classList.remove("graph-bubble__opened");
        ...
        changeElementColors(
            bubbleTriangle,
            "bubble-triangle__accessability-mode"
        );
    }

    // The pie chart bubble
    if ($finalBubbleButton.classList.contains("card-graph__opened"))
        $finalBubbleButton.classList.remove("card-graph__opened");
    if ($pieChartGraphContainer.classList.contains("card__graph__opened"))
        $pieChartGraphContainer.classList.remove("card__graph__opened");
    if ($finalBubble.classList.contains("graph-bubble__opened"))
        $finalBubble.classList.remove("graph-bubble__opened");
}

function removingUpsideDownBubblesClass(graphBubble, bubbleTriangle) {
    if (graphBubble.classList.contains("graph-bubble__upside-down"))
        graphBubble.classList.remove("graph-bubble__upside-down");
    if (bubbleTriangle.classList.contains("graph-bubble-triangle__upside-down"))
        bubbleTriangle.classList.remove("graph-bubble-triangle__upside-down");
}
    ...
    for (let i = 0; i < $cardGraphBubbles.length; i++) {
        ...
        if (graphBubble.classList.contains("graph-bubble__opened")) {
            ...
            if (bubbleRect.top - upsideFactor < navbarRect.bottom) {
                if (
                    !graphBubble.classList.contains("graph-bubble__upside-down")
                )
                    graphBubble.classList.add("graph-bubble__upside-down");
                if (
                    !bubbleTriangle.classList.contains(
                        "graph-bubble-triangle__upside-down"
                    )
                )
                    bubbleTriangle.classList.add(
                        "graph-bubble-triangle__upside-down"
                    );
            } else {
                removingUpsideDownBubblesClass(graphBubble, bubbleTriangle);
...
for (let i = 0; i < $selectButtons.length; i++) {
    ...
    selectButton.addEventListener("click", (e) => {
        ...
        if (!openedSelectLists[i]) {
            ...
            $selectionButtonTriangles[i].classList.add(
                "open-options__triangle__opened"
            );
        }
        ...
        for (let select of $selectLists[i].querySelectorAll("span")) {
            if (
                select.innerHTML ===
                selectButton.querySelector(".small-headline").innerHTML
            ) {
    ...
    for (let i = 0; i < $selectListsText.length; i++) {
        ...
        selectListsText.addEventListener("click", (e) => {
            ...
            $selectButtonsText[selectButtonIndex].innerHTML =
                selectListsText.innerHTML;
            if (i < 4)
                redrawFirstByTimeGraph(
                    selectListsText.innerHTML,
                    isAccessibilityOn
...
function closingSelectComponents() {
    for (let i = 0; i < $selectLists.length; i++) {
        if ($selectLists[i].classList.contains("selection-list__opened"))
            $selectLists[i].classList.remove("selection-list__opened");
        if (
            $selectionButtonTriangles[i].classList.contains(
                "open-options__triangle__opened"
            )
        )
            $selectionButtonTriangles[i].classList.remove(
                "open-options__triangle__opened"
            );

        for (let select of $selectLists[i].querySelectorAll("span")) {
            if (select.classList.contains("selected-option"))
                select.classList.remove("selected-option");
        }
    }
}
...
// Accessibility button
let isAccessibilityOn =
    Cookies.get("isAccessibilityOn") === "false" ? false : true;
...
const constIterateOverElements = () => {
    changeElementColors(
        $accessibilityButton,
        "accessibility-button__accessability-mode"
    );
    ...
    $graphContainers.forEach((container) => {
        ...
        changeElementColors(
            container,
            "graph-container-shadow__accessability-mode"
        );
    });
    ...
    $tableInputs.forEach((input) => {
        ...
        changeElementColors(
            input.parentNode.querySelector("svg"),
            "svg__accessability-mode"
        );
    });
...
const changeGraphsLegendsColors = () => {
    if (!isAccessibilityOn) {
        $darkTealLegendCircle.forEach((legend) =>
            removeClass(legend, "orange-legend__accessability-mode")
        );
        $greyLegendCircle.forEach((legend) =>
            removeClass(legend, "green-legend__accessability-mode")
        );
        $lightBlueLegendCircle.forEach((legend) =>
            removeClass(legend, "blue-legend__accessability-mode")
        );
        $greenLegendCircle.forEach((legend) =>
            removeClass(legend, "orange-legend__accessability-mode")
        );
    } else {
        $darkTealLegendCircle.forEach((legend) =>
            addClass(legend, "orange-legend__accessability-mode")
        );
        $greyLegendCircle.forEach((legend) =>
            addClass(legend, "green-legend__accessability-mode")
        );
        $lightBlueLegendCircle.forEach((legend) =>
            addClass(legend, "blue-legend__accessability-mode")
        );
        $greenLegendCircle.forEach((legend) =>
            addClass(legend, "orange-legend__accessability-mode")
        );
