...
import { getPointsByStartAndEndDates, getPointsByStartDateAndDuration } from '../utils/getDataPoints.js';

const url = 'http://localhost:3000/first-by-time-data';
...
const redrawFirstByTimeGraph = (durationStr, isAccessibilityOn) => {
    ...
    switch (durationStr) {
        case 'עד עכשיו': // All time
            ...
            areaPoints = getPointsByStartAndEndDates(firstByTimeData[0].allTimeFirstSeriesPoints, startingDate, endDate);
            greyColumnPoints = getPointsByStartAndEndDates(firstByTimeData[0].allTimeSecondSeriesPoints, startingDate, endDate);
            greenColumnPoints = getPointsByStartAndEndDates(firstByTimeData[0].allTimeThirdSeriesPoints, startingDate, endDate);
        ...
        case 'חודש אחרון': // Last month
            ...
            month = date.getMonth();
            endDate = '' + date.getDate() + '.' + ++month;
            areaPoints = getPointsByStartAndEndDates(firstByTimeData[0].lastMonthFirstSeriesPoints, startingDate, endDate);
            greyColumnPoints = getPointsByStartAndEndDates(firstByTimeData[0].lastMonthSecondSeriesPoints, startingDate, endDate);
            greenColumnPoints = getPointsByStartAndEndDates(firstByTimeData[0].lastMonthThirdSeriesPoints, startingDate, endDate);
            break;
        case 'שבועיים אחרונים': // Last two weeks
            startingDate = getDateOfNumberOfDaysAgo(14);
            areaPoints = getPointsByStartDateAndDuration(firstByTimeData[0].lastTwoWeeksFirstSeriesPoints, startingDate, 14);
            greyColumnPoints = getPointsByStartDateAndDuration(firstByTimeData[0].lastTwoWeeksSecondSeriesPoints, startingDate, 14);
            greenColumnPoints = getPointsByStartDateAndDuration(firstByTimeData[0].lastTwoWeeksThirdSeriesPoints, startingDate, 14);
            break;
        case 'שבוע אחרון': // Last week
            startingDate = getDateOfNumberOfDaysAgo(7);
            areaPoints = getPointsByStartDateAndDuration(firstByTimeData[0].lastWeekFirstSeriesPoints, startingDate, 7);
            greyColumnPoints = getPointsByStartDateAndDuration(firstByTimeData[0].lastWeekSecondSeriesPoints, startingDate, 7);
            greenColumnPoints = getPointsByStartDateAndDuration(firstByTimeData[0].lastWeekThirdSeriesPoints, startingDate, 7);
            break;
    }
            ...
            labels: {
                ...
                formatter: function(value) {
                    ...
                    for (let i = 0; i < areaPoints.length; i++) {
                        ...
                        if (areaPointDate.getMonth() === currentPoint.getMonth() && areaPointDate.getDate() === currentPoint.getDate()) {
                            areaPoints[i] = {x: areaPoints[i][0], y:  areaPoints[i][1], marker: {enabled: true,fillColor:lineColor,lineColor:lineColor,radius:4,states:{hover:{radius:8}}}}
                        }
                    }
                    for (let i = 0; i < greyColumnPoints.length; i++) {
                        const greyColumnPointDate = new Date(greyColumnPoints[i][0]);
                        const currentPoint = new Date(value.value);
                        if (greyColumnPointDate.getMonth() === currentPoint.getMonth() && greyColumnPointDate.getDate() === currentPoint.getDate()) {
                            greyColumnPoints[i] = {x: greyColumnPoints[i][0], y: greyColumnPoints[i][1], marker: {enabled: true,fillColor:firstColumnColor,lineColor:firstColumnColor,radius:4,states:{marker:{fillColor:firstColumnColor,lineColor:"#"},hover:{radius:8}}}}
                        }
                    }
        ...
        yAxis: [{
            ...
            crosshair: {
                ...
                label: {
                    ...
                    shape: 'square',
                    formatter: function(value) {
                        return '<div style="direction: ltr; padding: 10px">'
        ...
        tooltip: {
            ...
            formatter: function() {
                ...
                if (!isAccessibilityOn) {
                    ...
                    '<span style="color: #48caff;">' + areaPoint  + ' מאומתים מצטבר' + '</span>' +
                    '<span style="color: #898989;">' + greyColumnPoint  + ' מחלימים חדשים' + '</span>' +
                    '<span style="color: #1c7d7e;">' + greenColumnPoint  + ' מאומתים חדשים' + '</span>' +
                    '</div>'
                } else {
                    ...
                    '<span style="color: #2cd2db;">' + areaPoint  + ' מאומתים מצטבר' + '</span>' +
                    '<span style="color: #9ffa82;">' + greyColumnPoint  + ' מחלימים חדשים' + '</span>' +
                    '<span style="color: #fd8264;">' + greenColumnPoint  + ' מאומתים חדשים' + '</span>' +
                    '</div>'
                }
            }
        },
