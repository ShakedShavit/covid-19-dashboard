...
const url = 'http://localhost:3000/second-by-time-data';
...
const redrawSecondByTimeGraph = (durationStr, isAccessibilityOn) => {
    ...
    switch (durationStr) {
        case 'עד עכשיו': // All time
            ...
            greenLinePoints = getPointsByStartAndEndDates(secondByTimeData[0].allTimeFirstSeriesPoints, startingDate, endDate);
            blueLinePoints = getPointsByStartAndEndDates(secondByTimeData[0].allTimeSecondSeriesPoints, startingDate, endDate);
            break;
        case 'חודש אחרון': // Last month
            ...
            month = date.getMonth();
            endDate = '' + date.getDate() + '.' + ++month;
            greenLinePoints = getPointsByStartAndEndDates(secondByTimeData[0].lastMonthFirstSeriesPoints, startingDate, endDate);
            blueLinePoints = getPointsByStartAndEndDates(secondByTimeData[0].lastMonthSecondSeriesPoints, startingDate, endDate);
            break;
        case 'שבועיים אחרונים': // Last two weeks
            startingDate = getDateOfNumberOfDaysAgo(14);
            greenLinePoints = getPointsByStartDateAndDuration(secondByTimeData[0].lastTwoWeeksFirstSeriesPoints, startingDate, 14);
            blueLinePoints = getPointsByStartDateAndDuration(secondByTimeData[0].lastTwoWeeksSecondSeriesPoints, startingDate, 14);
            break;
        case 'שבוע אחרון': // Last week
            startingDate = getDateOfNumberOfDaysAgo(7);
            greenLinePoints = getPointsByStartDateAndDuration(secondByTimeData[0].lastWeekFirstSeriesPoints, startingDate, 7);
            blueLinePoints = getPointsByStartDateAndDuration(secondByTimeData[0].lastWeekSecondSeriesPoints, startingDate, 7);
            break;
    }
            ...
            labels: {
                ...
                formatter: function(value) {
                    ...
                    for (let i = 0; i < greenLinePoints.length; i++) {
                        ...
                        if (greenPointDate.getMonth() === currentPoint.getMonth() && greenPointDate.getDate() === currentPoint.getDate()) {
                            greenLinePoints[i] = {x: greenLinePoints[i][0], y:  greenLinePoints[i][1], marker: {enabled: true,fillColor:firstLineColor,lineColor:firstLineColor,radius:4,states:{hover:{radius:8}}}}
                            blueLinePoints[i] = {x: blueLinePoints[i][0], y:  blueLinePoints[i][1], marker: {enabled: true,fillColor:secondLineColor,lineColor:secondLineColor,radius:4,states:{hover:{radius:8}}}}
                        }
                    }
        ...
        tooltip: {
            ...
            formatter: function() {
                ...
                if (!isAccessibilityOn) {
                    ...
                    '<span style="color: #b6ca51;">' + greenLinePoint  + ' חולים קשים' + '</span>' +
                    '<span style="color: #48caff;">' + blueLinePoint  + ' מונשמים' + '</span>' +
                    '</div>'
                } else {
                    ...
                    '<span style="color: #fd8264;">' + greenLinePoint  + ' חולים קשים' + '</span>' +
                    '<span style="color: #48caff;">' + blueLinePoint  + ' מונשמים' + '</span>' +
                    '</div>'
                }
            }
